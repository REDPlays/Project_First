...
function Block:Spawn(user, target, conditionalData)
    if not target then
        return
    end
    ...
    local HitEffects = MeleeVFX.Block:Clone()
    HitEffects.Transparency = 1
    HitEffects.CFrame = targetRoot.CFrame * CFrame.new(0, 0, -1.5)
    ...
    local randomPitch = 90

    local sfx = MeleeSounds.Block:Clone()
